...
## Commands
- `docker compose -f deploy/docker-compose.yml --env-file .env up` - 핵심 서비스만 (backend, postgres, frontend, nginx)
- `docker compose -f deploy/docker-compose.yml --env-file .env --profile monitoring up` - Prometheus+Grafana 포함
